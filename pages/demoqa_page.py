class DemoQAPage:
    def __init__(self, page):
        ...
        self.book_rows = page.get_by_role("link").filter(has_text="")
    ...
    def get_visible_book_count(self):
        try:
            self.page.get_by_role("link", name="Git Pocket Guide").wait_for(
                state="visible", timeout=10000
            )
        except:
            pass
        return self.page.locator(".rt-tr-group .rt-td a").count()

    def is_book_visible(self, title):
        try:
            self.page.get_by_role("link", name=title).wait_for(
                state="visible", timeout=10000
            )
            return True
        except:
